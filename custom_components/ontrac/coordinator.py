"""Coordinator for the OnTrac parcel tracker integration.
...
from .api import OnTracApiClient, OnTracApiError
from .const import (
...
class OnTracCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def __init__(
        ...
        # Tracking codes we've already warned about disappearing (a resolved
        # parcel starting to 404) — one-shot per code, cleared alongside the
        # raw cache when a parcel is no longer tracked.
        self._warned_disappeared: set[str] = set()
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        self._warned_disappeared &= tracked_codes
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (OnTracApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, OnTracApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("OnTrac fetch failed for %s: %s", code, result)
            ...
            if result is None:
                ...
                previously_resolved = self._raw_cache.get(code)
                if previously_resolved is not None and code not in self._warned_disappeared:
                    self._warned_disappeared.add(code)
                    _LOGGER.warning(
                        "OnTrac stopped returning a previously-resolved parcel "
                        "for a tracked code — open an issue: "
                        "https://github.com/ha-parcel-integrations/ha-ontrac/"
                        "issues/new?template=unrecognised_status.yml"
                    )
                raws_by_code[code] = previously_resolved or {"Tracking": code}
                continue

            # The response's own tracking number can be missing on edge
            # payloads; fall back to the code we asked for so the sensor keeps
            # its key.
            result.setdefault("Tracking", code)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "OnTrac rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("OnTrac unreachable for all tracked parcels")
        ...
        normalized_active = sort_parcels_by_ts(active, "planned_to")
