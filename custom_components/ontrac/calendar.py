"""Calendar platform for the OnTrac parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the OnTrac deliveries calendar from a config entry."""
    async_add_entities([OnTracDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class OnTracDeliveriesCalendar(CoordinatorEntity[OnTracCoordinator], CalendarEntity):
    """A read-only calendar of expected OnTrac deliveries.
    ...
    def __init__(self, coordinator: OnTracCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            start = parse_iso(parcel.get("planned_from") or parcel.get("planned_to"))
            if start is None:
                continue
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "OnTrac parcel")
            description_parts = [
