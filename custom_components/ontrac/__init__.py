"""OnTrac parcel tracker custom component for Home Assistant."""
...
from .api import OnTracApiClient
...
@dataclass
class OnTracData:
    """Runtime data attached to the OnTrac config entry."""

    client: OnTracApiClient
    coordinator: OnTracCoordinator


type OnTracConfigEntry = ConfigEntry[OnTracData]


async def async_setup_entry(hass: HomeAssistant, entry: OnTracConfigEntry) -> bool:
    """Set up OnTrac from a config entry."""
    # No auth: OnTrac tracking is public, so the HA-managed session is fine.
    client = OnTracApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = OnTracData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: OnTracConfigEntry) -> bool:
    """Unload the OnTrac config entry."""
