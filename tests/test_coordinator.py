"""Tests for the OnTrac coordinator: fetching, caching and events.
...
from custom_components.ontrac.api import OnTracApiError
...
from custom_components.ontrac.coordinator import OnTracCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["Events"] = [
        event("ARRD", "2026-03-18T20:00:00+00:00", short_desc="Arrived at Facility")
    ]
    return sample
...
async def test_update_merges_multiple_parcels(hass):
    ...
    coordinator = OnTracCoordinator(hass, client, entry)
...
async def test_update_warns_once_when_a_resolved_parcel_starts_404ing(hass, caplog):
    """Uses an active (not delivered) code — a delivered one is skipped from
    the fetch entirely on the next cycle, so it would never reach the API to
    ...
    coordinator = OnTracCoordinator(hass, client, entry)
    await coordinator._async_update_data()  # resolves once, populates the cache

    client.async_get_parcel.return_value = None  # now 404s
    await coordinator._async_update_data()
    await coordinator._async_update_data()  # second 404 must not warn again

    assert caplog.text.count("previously-resolved parcel") == 1


async def test_update_never_resolved_does_not_warn_on_404(hass, caplog):
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    ...
    assert "previously-resolved parcel" not in caplog.text
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = OnTracApiError("HTTP 500")
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    moved = active_sample()
    moved["UtcExpectedDeliveryDateTime"] = "2026-03-20T21:00:00-04:00"
    client.async_get_parcel.return_value = moved
    await coordinator._async_update_data()
    await hass.async_block_till_done()

    assert len(events) == 1
    assert events[0].data["old_planned_to"] == "2026-03-19T21:00:00-04:00"
    assert events[0].data["new_planned_to"] == "2026-03-20T21:00:00-04:00"
...
async def test_losing_the_eta_is_silent(hass):
    ...
    dropped["UtcExpectedDeliveryDateTime"] = None
