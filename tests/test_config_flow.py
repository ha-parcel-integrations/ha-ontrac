"""Tests for the OnTrac config and options flow."""
from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.ontrac.config_flow import (
...
def _hub(parcels: list[dict], **extra_options) -> MockConfigEntry:
    return MockConfigEntry(
        domain=DOMAIN,
        unique_id=DOMAIN,
        options={CONF_PARCELS: parcels, **extra_options},
    )
...
async def test_options_shows_parcel_and_settings_menu(hass):
    entry = _hub([])
...
async def test_options_add_parcel(hass):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _parcel_input("example123456")
    )
    assert result["type"] == "create_entry"
...
async def test_options_add_code_with_separators(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _parcel_input("example-123 456")
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [
        {CONF_TRACKING_CODE: "EXAMPLE123456"}
...
async def test_options_add_invalid_tracking_code(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _parcel_input("abc")
    )
...
async def test_options_de_duplicates_tracking_codes(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _parcel_input("EXAMPLE111111", "example111111")
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [
...
async def test_options_remove_parcel(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _parcel_input("EXAMPLE222222")
    )
    assert result["type"] == "create_entry"
...
async def test_options_replaces_the_tracked_code_list(hass):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        _parcel_input("EXAMPLE222222", "EXAMPLE333333"),
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [
        {CONF_TRACKING_CODE: "EXAMPLE222222"},
        {CONF_TRACKING_CODE: "EXAMPLE333333"},
    ]


async def test_options_parcels_preserves_settings(hass):
    entry = _hub(
        [],
        **{
...
async def test_options_settings_preserves_parcels(hass):
    entry = _hub([{CONF_TRACKING_CODE: "EXAMPLE111111"}])
    entry.add_to_hass(hass)
